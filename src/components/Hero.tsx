    ...
    <section id="inicio" className="relative min-h-screen flex items-center pt-20 overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          ...
          <p className="text-lg md:text-xl text-emerald-50 mb-8 leading-relaxed max-w-lg mx-auto md:mx-0">
            Aqui no Alto da Serra, você encontra descanso, charme e experiências que renovam o corpo e a alma.
          </p>

          <div className="flex flex-col sm:flex-row gap-4 justify-center md:justify-start">
            <Button size="lg" className="bg-emerald-500 hover:bg-emerald-600 text-white rounded-full px-8 h-14 text-lg font-semibold group w-full sm:w-auto">
              Reserve agora
              <ChevronRight className="ml-2 h-5 w-5 group-hover:translate-x-1 transition-transform" />
            </Button>
            <Button size="lg" variant="outline" className="bg-white/10 hover:bg-white/20 text-white border-white/30 rounded-full px-8 h-14 text-lg w-full sm:w-auto">
              Ver Tarifas
            </Button>
          </div>
          ...
          <div className="mt-12 flex items-center justify-center md:justify-start gap-4 md:gap-6 text-emerald-100/80">
            ...
            <div className="flex flex-col items-center md:items-start">
              <span className="text-xl md:text-2xl font-bold text-white">🌿 Verde</span>
              <span className="text-xs md:text-sm">Serenidade</span>
            </div>
            <div className="w-px h-10 bg-emerald-400/30" />
            <div className="flex flex-col items-center md:items-start">
              <span className="text-xl md:text-2xl font-bold text-white">🌄 Vistas</span>
              <span className="text-xs md:text-sm">Inesquecíveis</span>
            </div>
          </div>
        </motion.div>
      </div>
